...
local function loadSettings()
	...
	if not success then 
		if writefile then
			game.StarterGui:SetCore("SendNotification", {
    Title = "Notification GUI : ";
    Text = "Rayfield had an issue accessing configuration saving capability";
    Duration = 2;
})
		end
	end
...
if debugX then
	game.StarterGui:SetCore("SendNotification", {
    Title = "Notification GUI : ";
    Text = "Now Loading Settings Configuration";
    Duration = 2;
})
end
...
if not requestsDisabled then
	if debugX then
		noti('Querying Settings for Reporter Information')
	end
	local function sendReport()
		if useStudio then
			noti('Sending Analytics')
		else
			...
			task.spawn(function()
				...
				if success and reporter then
					...
					noti("Failed to load or execute the reporter. \nPlease notify Rayfield developers at sirius.menu/discord.")
				end
			end)
			if debugX then noti('Finished Report') end
		end
	end
...
if debugX then
	noti('Moving on to continue initialisation')
end
...
repeat
	...
	if not warned then
		moti('Rayfield | Build Mismatch')
		noti('Rayfield may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end
...
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/Bysuskhmerops62/AVG/refs/heads/main/icons.lua.txt')
-- Variables
...
local function getIcon(name : string): {id: number, imageRectSize: Vector2, imageRectOffset: Vector2}
	if not Icons then
		noti("Lucide Icons: Cannot use icons as icons library is not loaded")
		return
	end
	...
	if type(r[1]) ~= "number" or type(rirs) ~= "table" or type(riro) ~= "table" then
		noti("Lucide Icons: Internal error: Invalid auto-generated asset entry")
	end
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		noti("Rayfield | Cannot use Lucide icons as icons library is not loaded")
	else
		noti("Rayfield | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			noti("Rayfield | Unable to find '"..FlagName.. "' in the save file.")
			noti("The error above may not be an issue if new elements have been added or not been set values.")
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			noti('Rayfield | Not seeing your actions in notifications?')
			noti("Notification Actions are being sunset for now, keep up to date on when they're back in the discord. (sirius.menu/discord)")
		end
...
function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping ' Rayfield'.", Duration = 1, Image = 4400697855})
		else
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.rayfieldOpen.Value or 'K'}.`, Duration = 1, Image = 4400697855})
		end
	end
...
local function createSettings(window)
	if not (writefile and isfile and readfile and isfolder and makefolder) and not useStudio then
		...
		noti('Can\'t create settings as no file-saving functionality is available.')
		return
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.Theme then
		...
		if not success then
			...
			if not success then
				noti('CRITICAL ERROR - NO DEFAULT THEME')
				print(result2)
			end
			noti('issue rendering theme. no theme on file')
			print(result)
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					noti("Rayfield | "..Key.." Error " ..tostring(Response))
					noti('Check docs.sirius.menu for help with Rayfield specific development.')
				end
			end
		end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			music(1673280232)
				...
				if not Success then
					...
					Button.Title.Text = "Error : "..tostring(Response)
		...
		function Tab:CreateInput(InputSettings)
			...
			music(1673280232)
				...
				if not Success then
					...
					Input.Title.Text = "Error : "..tostring(Response)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							Keybind.Title.Text = "Error : "..tostring(Response)
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			music(1673280232)
				...
				if not Success then
					...
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
					Toggle.Title.Text = "Error : "..tostring(Response)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								Slider.Title.Text = "Error : "..tostring(Response)
...
function RayfieldLibrary:LoadConfiguration()
	...
	if CEnabled then
		...
		elseif not success and not notified then
			noti('Rayfield Configurations Error | '..tostring(result))
